...
func main() {
	...
	e.DELETE("/api/events/:id/sheets/:rank/:num/reservation", func(c echo.Context) error {
		...
		// 複合indexは入っている。
		if err := tx.QueryRow("SELECT id, user_id, reserved_at FROM reservations WHERE event_id = ? AND sheet_id = ? AND canceled_at IS NULL GROUP BY event_id HAVING reserved_at = MIN(reserved_at)", event.ID, sheet.ID).Scan(&reservation.ID, &reservation.UserID, &reservation.ReservedAt); err != nil {
			tx.Rollback()
			if err == sql.ErrNoRows {
	...
	e.GET("/admin/api/reports/events/:id/sales", func(c echo.Context) error {
		...
		rows, err := db.Query("SELECT r.*, s.rank AS sheet_rank, s.num AS sheet_num, s.price AS sheet_price, e.price AS event_price FROM reservations r INNER JOIN sheets s ON s.id = r.sheet_id INNER JOIN events e ON e.id = r.event_id WHERE r.event_id = ? ORDER BY reserved_at ASC", event.ID) // FOR UPDATE
		if err != nil {
			return err
		}
		defer rows.Close()
...
func renderReportCSV(c echo.Context, reports []Report) error {
	...
	fmt.Fprint(&builder, "reservation_id,event_id,rank,num,price,user_id,sold_at,canceled_at\n")
	for _, v := range reports {
		fmt.Fprintf(&builder, "%d,%d,%s,%d,%d,%d,%s,%s\n",
	...
	_, err := io.Copy(c.Response(), strings.NewReader(body))
	return err
}
